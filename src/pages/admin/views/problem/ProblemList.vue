<template>
  <div class="view">
    ...
    <el-dialog :title="$t('m.Add_Problem')"
               v-if="contestId"
               width="80%"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteProblem (id) {
        this.$confirm(this.$t('m.Sure_to_delete_this_problem'), this.$t('m.Delete_Problem'), {
          type: 'warning'
        }).then(() => {
      ...
      makeContestProblemPublic (problemID) {
        this.$prompt(this.$t('m.Please_input_display_id_for_public_problem'), this.$t('m.confirm')).then(({value}) => {
